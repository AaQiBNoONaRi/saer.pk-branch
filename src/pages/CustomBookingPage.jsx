    ...
    const fetchAccounts = async () => {
      try {
        ...
        if (res.ok) {
          ...
          const brAccs = data.filter(a => a.account_type === 'Branch');
          setBeneficiaryAccounts(orgAccs.map(a => `${a.account_title ? a.account_title + ' - ' : ''}${a.bank_name} (Acc: ${a.account_number})`));
          setAgentAccounts(brAccs.map(a => `${a.account_title ? a.account_title + ' - ' : ''}${a.bank_name} (Acc: ${a.account_number})`));
        }
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-3xl p-6 shadow-sm">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-[2px] mb-1">Payment Sent By</p>
              <h3 className="text-sm font-black text-slate-900 uppercase tracking-tight">
                {(() => {
                  ...
                  if (branchData) {
                    ...
                    return `${branch.branch_name || 'Branch'} (Branch)`;
                  }
                  return localStorage.getItem('branch_name') || 'Branch';
                })()}
              </h3>
            </div>
          </div>
          <div className="text-right">
            <span className="px-3 py-1 bg-blue-50 text-blue-600 rounded-lg text-[10px] font-black uppercase tracking-wider">
              {localStorage.getItem('employee_data') ? 'Employee' : 'Branch'}
            </span>
          </div>
        </div>
      </div>
        ...
        {(paymentMethod === 'bank' || paymentMethod === 'cheque') && (
          <div className="space-y-4 pt-4 border-t border-slate-100">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <SelectField label="Branch Account" options={agentAccounts.length > 0 ? agentAccounts : ['No accounts found']} value={paymentData.agentAccount} onChange={v => upd('agentAccount', v)} />
            </div>
      ...
      const fetchBooking = async () => {
        try {
          ...
          if (res.ok) {
            ...
            setCurrentStep(3);
            window.history.pushState({ step: 3 }, '', '/custom-booking/step-3');
          }
        } catch (err) {
